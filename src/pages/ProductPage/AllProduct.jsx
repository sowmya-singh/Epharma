                ...
                <>
                    <Box p={"15px"} bgColor="#ffffff" borderRadius={"10px"} >
                            ...
                            {prod.map((el) => {
                                ...
                                return <GridItem key={id} w="100%" p={'14px 16px 15px 16px'} border='solid 1px rgba(112,112,112,.38)' borderRadius={"8px"} position="relative">
                                    ...
                                    <Box overflow={"hidden"} h='30px' w="100%" mt="5px" >
                                        <Text as="span" color={"#395B64"} fontSize="10px" borderRadius={"40px"} mr="4px" p='4px 10px' bgColor={"#f3f7fb"}>{category}</Text>
                                        <Text as="span" color={"#395B64"} fontSize="10px" borderRadius={"40px"} mr="4px" p='4px 10px' w="max-content" bgColor={"#f3f7fb"}>{sub_category}</Text>
                                    </Box>
                                    ...
                                    <Button w="100%" bgColor={"#395B64"} color="white" textTransform={"uppercase"} _hover={{ bgColor: "#395B64", color: "white" }} onClick={() => {
                                        handleAdd(el)
                                    }} >Add to cart</Button>
                                </GridItem >
                            })}
                        ...
                        {total >= 20 && <Box textAlign={"center"} mt="30px" display="flex" gap='5px' justifyContent={"center"}>
                            {page !== 1 && <Button size={"sm"} border="1px solid #d4d5d9" bgColor="white" p="0 20px" fontWeight={"400"} _hover={{ color: '#395B64', bgColor: 'white' }} fontSize={"13px"} color="#151b39" onClick={() => handlePage(page - 1)}>PREV</Button>}
                            {<Button onClick={(e) => handlePage(+e.target.innerHTML)} size={"sm"} _hover={{ color: '#395B64', bgColor: '#D4D5D940' }} bgColor="white" fontWeight={"400"} fontSize={"13px"} color="#151b39">{page}</Button>}
                            {page !== Math.ceil(total / 20) && <Button size={"sm"} border="1px solid #d4d5d9" bgColor="white" p="0 20px" fontWeight={"400"} _hover={{ color: '#395B64', bgColor: 'white' }} fontSize={"13px"} color="#151b39" onClick={() => handlePage(page + 1)}>NEXT</Button>}
                        </Box>}
                    </Box>
                </>
            )
